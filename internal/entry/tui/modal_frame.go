...
func renderPaddedModalFrame(boxW, boxH int, title, hint string, bodyLines []string) string {
	...
	// The hint sits inside the bottom border and, when too long, pushes the border past the frame width — truncate to the inner width so the top and bottom borders stay aligned.
