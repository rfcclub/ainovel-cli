...
func RunImport(ctx context.Context, st *store.Store, path string) (<-chan Event, error) {
	...
	go func() {
		...
		emit(StageImport, "Nhập hồ sơ mô phỏng...", nil)
		result, err := ImportProfile(ctx, st, path)
		if err != nil {
			emit(StageError, "Nhập hồ sơ mô phỏng thất bại", err)
			return
		}
		emit(StageDone, fmt.Sprintf("Hồ sơ mô phỏng đã nhập: thêm %d bài, bỏ qua %d bài trùng", result.ImportedSources, result.SkippedSources), nil)
	}()
	return events, nil
}
