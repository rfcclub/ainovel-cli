...
func (s *ChapterRecordStore) Load(chapter int) (*domain.ChapterRecord, error) {
	...
	if record.Chapter != chapter {
		return nil, fmt.Errorf("đọc bản ghi tiếp nhận chương %d: bản ghi có số chương là %d", chapter, record.Chapter)
	}
	return &record, nil
}
...
// Prepare builds the next version of chapter records without persisting, letting cross-record invariants be validated before the write.
...
func prepareChapterRecord(existing *domain.ChapterRecord, chapter int, origin domain.ChapterOrigin, content string, facts domain.ChapterFacts, style domain.StyleDelta) (*domain.ChapterRecord, bool) {
	...
	revision := 1
	if existing != nil {
		// Overwriting an old record preserves the foreshadows this chapter planted itself: a rewrite only swaps the prose and must not erase this chapter's planting facts.
