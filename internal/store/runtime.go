...
// RuntimeStore manages the unified runtime queue and per-task logs.
...
// AppendQueue appends one runtime-queue record, assigning an increasing sequence number automatically.
...
// LoadQueue reads every currently persisted runtime-queue item.
...
// LoadQueueAfter returns queue items after the given sequence number.
...
// AppendTaskLog appends one task's run log.
...
// LoadTaskLog reads one task's whole run log.
...
// Reset clears the runtime queue and task logs.
